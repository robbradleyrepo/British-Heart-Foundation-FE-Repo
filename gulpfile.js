...
// images
// DISABLED due to latest npm version
...
gulp.task('scripts', ['lint'], function () {
	...
	browserify({ basedir: './', debug: true })
		...
		.plugin(tsify, { "skipLibCheck": true, "incremental": true, noImplicitAny: true, target: 'es5', "types": ["node"], "typeRoots": ["node_modules/@types"] })
		.bundle()
...
gulp.task('styles-legacy', function () {
	sass('src/sass/styles-legacy.scss', { sourcemap: true })
		.on('error', sass.logError)
		.pipe(gulpif(options.env == environments.dev, sourcemaps.write()))
...
gulp.task('build', function (cb) {
	...
	runSequence(['styles', 'styles-legacy', 'scripts', 'scripts-forms', 'images', 'pages'], cb);
});
...
// default task
gulp.task('default', ['clean', 'lint', 'scripts', 'scripts-forms', 'fonticon', 'fonts', 'styles', 'styles-legacy', 'images', 'serve', 'watch']);